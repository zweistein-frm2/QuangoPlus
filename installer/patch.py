...
class PatchSet(object):
    ...
    def parse(self, stream):
      ...
      # each parsing block already has line available in fe.line
        ...
        while fe.next():
            ...
            # hunkskip and hunkbody code skipped until definition of hunkhead is
            # parsed
            if hunkbody:
    ...
    def _detect_type(self, p):
        ...
        if (len(p.header) > 1 and p.header[-2].startswith(b"Index: ") and p.header[-1].startswith(b"=" * 67)):
            return SVN
        ...
        if len(p.header) > 1:
            ...
            if p.header[idx].startswith(b'diff --git a/'):
                if (idx + 1 < len(p.header) and re.match(b'index \\w{7}..\\w{7} \\d{6}', p.header[idx + 1])):
                    if DVCS:
                        return GIT
    ...
    def diffstat(self):
        ...
        output += (" %d files changed, %d insertions(+), %d deletions(-), %+d bytes" % (len(names), sum(insert), sum(delete), delta))
        return output
...
def main():
    ...
    opt.add_option("-q", "--quiet", action="store_const",
                   dest="verbosity", const=0, help="print only warnings and errors", default=1)
    ...
    opt.add_option("-d", "--directory", metavar='DIR', help="specify root directory for applying patch")
    opt.add_option("-p", "--strip", type="int", metavar='N', default=0, help="strip N path components from filenames")
    opt.add_option("--revert", action="store_true", help="apply patch in reverse order (unpatch)")
    ...
    if readstdin:
        ...
        if (':' in patchfile and urltest.isalpha() and len(urltest) > 1): # one char before : is a windows drive letter
    ...
    # todo: document and test line ends handling logic - patch.py detects proper
    # line-endings
    #       for inserted hunks and issues a warning if patched file has
    #       incosistent line ends
